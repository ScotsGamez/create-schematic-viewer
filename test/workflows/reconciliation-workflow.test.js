...
test("reconciliation attaches only an absent bundle and fails closed on conflicts", async () => {
  ...
  assert.ok(plan >= 0 && plan < auth && auth < attach);
  ...
  assert.match(workflow, /oras discover[\s\S]+--artifact-type "\$BUNDLE_MEDIA_TYPE"/);
  assert.match(workflow, /\(\.referrers \/\/ \[\]\) \| length/);
  assert.doesNotMatch(
    workflow,
  ...
  assert.equal(
    (
      workflow.match(
        /if: steps\.registry_plan\.outputs\.action == 'attach'/g,
      ) ?? []
    ).length,
    2,
  );
  ...
  assert.match(workflow, /--distribution-spec v1\.1-referrers-api/);
...
test("reconciliation has no release, image, tag, or signing mutation path", async () => {
  const workflow = await reconciliationWorkflow();
  const forbidden = [
    /actions\/attest@/,
...
test("reconciliation proves both stores, unchanged tags, and cleans credentials", async () => {
  const workflow = await reconciliationWorkflow();
  const attach = workflow.indexOf("Attach the verified bundle to GHCR");
  ...
  assert.ok(attach >= 0 && verify > attach && summary > verify && cleanup > summary);
