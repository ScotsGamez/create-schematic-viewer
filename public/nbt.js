...
async function parseNbt(input) {
  const buffer = await decompressIfNeeded(input);
...
async function decompressIfNeeded(input) {
  ...
  if (bytes.length >= 2 && bytes[0] === 0x1f && bytes[1] === 0x8b) {
    return decompressStream(bytes, "gzip");
  }
...
async function decompressStream(bytes, format) {
  if (!("DecompressionStream" in window)) {
    if (format === "gzip") {
      throw new Error("This browser cannot decompress gzip NBT files. Try a current Chrome, Edge, or Firefox.");
    }
